...
- **Settings panel** — Organized into 6 card-based sections: Libraries (list with model count + last scanned timestamp, per-library scan, add form), Appearance (theme toggle, thumbnail mode, favorites-first, collection tint), Printing (slicer integration, bed overlay toggle, printer preset, dimensions, shape), Maintenance (auto-tag on scan, bulk regen/auto-tag/extract-metadata with progress bars), Advanced (collapsible, import credentials only), Updates (check/apply). Each section is a visually distinct card with accent-colored uppercase header.
